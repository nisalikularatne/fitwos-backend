...
exports.getRooms = async ({
    ...
    return Room.query().modify(builder => {
        ...
        (is_equipment || category_filters) && 
            builder.whereIn('room_uuid',
                Room.query().distinct('room_uuid').joinRelated('tabata_workouts.exercises.categories').modify(builder => {
                    is_equipment && builder.where('tabata_workouts:exercises.is_equipment','=',is_equipment);
                    category_filters && builder.whereIn('tabata_workouts:exercises:categories.name',category_filters);
                })
            );
        builder && builder.page(Number(page) - 1, page_size);
        builder.orderBy(sort, order);
        
    }).where({is_deleted: false}).withGraphFetched('[user,tabata_workouts.exercises.[categories]]');
}
